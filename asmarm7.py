...
def usage():
    print("Uso: python genasm.py <archivo.c> <opt_level: 0|1|2|3|s|g> <salida.s> <comments:on|off> <folding:on|off> <simplify:on|off>")
    sys.exit(1)
...
cmd = ["arm-none-eabi-gcc", "-S", f"-O{opt}", "-mcpu=arm7tdmi", "-mthumb-interwork"]
if vasm:
    cmd.append(vasm)
...
try:
    ...
    print(f"✅ Ensamblador generado en '{out}' con -O{opt}, comentarios: {cmts}, folding: {folding}")
...
if simplify == "on":
    try:
        ...
        simplified = []
        skip_prefixes = (".cpu", ".arch", ".fpu", ".eabi_attribute", "@ GGC heuristics:")
