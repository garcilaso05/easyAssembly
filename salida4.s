...
@ test1_nofold.c:7:     int division = 4 / A;
	ldr	r1, [fp, #-8]	@, A
	mov	r0, #4	@,
	bl	__aeabi_idiv		@
	mov	r3, r0	@ division_7,
	str	r3, [fp, #-24]	@ division_7, division
@ test1_nofold.c:8:     int modulo = 4 % A;
	mov	r3, #4	@ tmp131,
	ldr	r1, [fp, #-8]	@, A
	mov	r0, r3	@, tmp131
	bl	__aeabi_idivmod		@
...
@ test1_nofold.c:10:     int resultado = (suma + resta) * (multiplicacion - division);
