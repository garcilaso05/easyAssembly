...
def usage():
    print("Uso: python asmmips.py <archivo.c> <opt_level: 0|1|2|3|s|g> <salida.s> <comments:on|off> <folding:on|off> <simplify:on|off>")
    sys.exit(1)
...
# Ejecutar compilación para MIPS
cmd = ["mips-linux-gnu-gcc", "-S", f"-O{opt}", "-march=mips32"]
if vasm:
    cmd.append(vasm)
...
try:
    ...
    print(f"✅ Ensamblador MIPS generado en '{out}' con -O{opt}, comentarios: {cmts}, folding: {folding}")
