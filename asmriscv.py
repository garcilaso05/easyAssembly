...
def usage():
    print("Uso: python asmriscv.py <archivo.c> <opt_level: 0|1|2|3|s|g> <salida.s> <comments:on|off> <folding:on|off> <simplify:on|off>")
    sys.exit(1)
...
# Ejecutar compilación para RISC-V
cmd = ["riscv64-unknown-elf-gcc", "-S", f"-O{opt}", "-march=rv32imac"]
if vasm:
    cmd.append(vasm)
...
try:
    ...
    print(f"✅ Ensamblador RISC-V generado en '{out}' con -O{opt}, comentarios: {cmts}, folding: {folding}")
...
if simplify == "on":
    try:
        ...
        simplified = []
        skip_prefixes = (".file", ".section", ".ident", ".cfi_", ".type", ".size", "#", "//", "/*", "*/")
