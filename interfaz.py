...
class SelectWindow:
    def __init__(self, so):
        ...
        archs = ["ARMv7", "x86‑64", "RISC-V", "MIPS"]
    ...
    def start(self):
        ...
        if lang == "C" and arch == "ARMv7":
            ...
            CMIPSWindow(self.so)
        elif lang == "C" and arch == "RISC-V":
            CRISCVWindow(self.so)
        else:
...
class CArm7Window:
    ...
    def show_folding_info(self):
        ...
        tk.Button(win, text="OK", font=("Arial", 11, "bold"), bg="#2a2947b", fg="white", command=win.destroy).pack(pady=10)
...
class CX86Window:
    def __init__(self, so):
        ...
        self.output_entry.grid(row=5, column=1, columnspan=2, sticky="w")
        tk.Button(
            self.win, text="Compilar", font=("Arial", 12, "bold"), bg="#2a7b2a", fg="white",
            activebackground="#19325a", activeforeground="white", command=self.compile
        ).pack(pady=20)
    ...
    def show_dependencies_window(self, so):
        ...
        if so.startswith("Linux"):
            ...
            def copy_cmd():
                ...
                tk.messagebox.showinfo("Copiado", "Comando copiado al portapapeles.", parent=dep_win)
            tk.Button(dep_win, text="Copiar comando Homebrew", font=("Arial", 9, "bold"), fg="#2a4a7b", command=copy_cmd, width=30).pack(pady=2)
        tk.Button(dep_win, text="OK", font=("Arial", 11, "bold"), bg="#2a7b2a", fg="white", command=dep_win.destroy).pack(pady=10)
    ...
    def show_opt_info(self):
        ...
        tk.Label(win, text=msg, justify="left", anchor="w", font=("Arial", 10), fg=azul_oscuro, wraplength=400).pack(padx=10, pady=15)
        tk.Button(win, text="OK", font=("Arial", 11, "bold"), bg="#2a7b2a", fg="white", command=win.destroy).pack(pady=10)
...
class CMIPSWindow:
    ...
    def show_dependencies_window(self):
        dep_win = tk.Toplevel(self.win)
        dep_win.title("Dependencias necesarias")
        dep_win.geometry("500x200")
        dep_win.resizable(False, False)
        azul_oscuro = "#1a2942"
        fuente_titulo = font.Font(family="Arial", size=13, weight="bold")
        fuente_normal = font.Font(family="Arial", size=10)
        msg = (
            "Dependencias necesarias para C-MIPS\n\n"
            "Para compilar código C a ensamblador MIPS es necesario disponer de un compilador cruzado como 'mips-linux-gnu-gcc'.\n\n"
            "No existe un método único de instalación para todos los sistemas operativos. Es posible que debas buscar e instalar un toolchain precompilado de MIPS adecuado para tu sistema.\n\n"
            "Consulta la documentación de tu distribución o busca paquetes como 'mips-linux-gnu-gcc', 'mipsel-linux-gnu-gcc' o similares."
        )
        tk.Label(dep_win, text=msg, justify="left", anchor="w", font=fuente_normal, fg=azul_oscuro, wraplength=470).pack(padx=10, pady=15)
        tk.Button(dep_win, text="OK", font=("Arial", 11, "bold"), bg="#2a7b2a", fg="white", command=dep_win.destroy).pack(pady=10)
...
class CRISCVWindow:
    def __init__(self, so):
        self.so = so
        # Aviso experimental RISC-V
        messagebox.showwarning(
            "Aviso sobre RISC-V",
            "La función de conversión a RISC-V es experimental.\n\n"
            "Aunque se ha seguido documentación para implementarla, el desarrollador no ha tenido tiempo de testearla en profundidad y se desconoce si puede tener muchos errores."
        )
        self.win = tk.Toplevel()
        self.win.title("easyAssembly - C a Assembly (RISC-V)")
        self.win.geometry("600x500")
        azul_oscuro = "#1a2942"
        fuente_titulo = font.Font(family="Arial", size=16, weight="bold")
        fuente_normal = font.Font(family="Arial", size=11)
        tk.Label(self.win, text="C a Assembly (RISC-V)", font=fuente_titulo, fg=azul_oscuro).pack(pady=(10, 5))
        self.show_dependencies_window()
        # Recuadro comentario desplegable
        self.comment_frame = tk.Frame(self.win, bg="#e6f0fa", bd=2, relief="groove")
        self.comment_frame.pack(padx=10, pady=(5, 10), fill="x")
        ...
        tk.Label(frame, text="Archivo de salida:", font=fuente_normal, fg=azul_oscuro).grid(row=5, column=0, sticky="w")
        self.output_entry = tk.Entry(frame, width=30)
        self.output_entry.insert(0, "salida_riscv.s")
        self.output_entry.grid(row=5, column=1, columnspan=2, sticky="w")
        tk.Button(
            self.win, text="Compilar", font=("Arial", 12, "bold"), bg="#2a7b2a", fg="white",
            activebackground="#19325a", activeforeground="white", command=self.compile
    ...
    def show_dependencies_window(self):
        ...
        fuente_normal = font.Font(family="Arial", size=10)
        msg = (
            "Dependencias necesarias para C-RISC-V\n\n"
            "Para compilar código C a ensamblador RISC-V es necesario disponer de un compilador cruzado como 'riscv64-unknown-elf-gcc'.\n\n"
            "No existe un método único de instalación para todos los sistemas operativos. Es posible que debas buscar e instalar un toolchain precompilado de RISC-V adecuado para tu sistema.\n\n"
            "Consulta la documentación de tu distribución o busca paquetes como 'riscv64-unknown-elf-gcc' o similares."
        )
        tk.Label(dep_win, text=msg, justify="left", anchor="w", font=fuente_normal, fg=azul_oscuro, wraplength=470).pack(padx=10, pady=15)
        tk.Button(dep_win, text="OK", font=("Arial", 11, "bold"), bg="#2a7b2a", fg="white", command=dep_win.destroy).pack(pady=10)
    ...
    def get_dependencies_message(self, so):
        if so.startswith("Windows"):
            return (
                "Debes instalar el compilador RISC-V (riscv64-unknown-elf-gcc).\n"
                "1. Descarga el instalador desde el enlace inferior.\n"
                "2. Instala y añade a la variable PATH."
            )
        elif so.startswith("Linux"):
    ...
    def compile(self):
        ...
        if not src:
            messagebox.showerror("Error", "Debes seleccionar un archivo de entrada.")
            return
        cmd = ["python3", "asmriscv.py", src, opt, out, cmts, folding, simplify]
        try:
            subprocess.run(cmd, check=True)
            messagebox.showinfo("Éxito", f"Archivo ensamblador generado: {out}")
        except subprocess.CalledProcessError:
